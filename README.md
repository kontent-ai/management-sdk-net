...
> ℹ This is **the BETA version of the SDK** which uses [Management API version 2](https://kontent.ai/learn/reference/management-api-v2) ℹ.
...
💡 If you want to see all .NET related resources including REST API reference with .NET code samples for every endpoind, check out the ["Develop .NET apps" overview page](https://kontent.ai/learn/tutorials/develop-apps/overview?tech=dotnet).
...
You can catch Kontent errors (more in [error section in Management API reference](https://kontent.ai/learn/reference/management-api-v2#section/Errors))) by usin `try-catch` block and catching `Kentico.Kontent.Management.Exceptions.ManagementException`.
